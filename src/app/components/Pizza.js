...
const Pizza = ({ pizza }) => {
  ...
  return <div className="group py-2  px-4 xl:py-4 xl:px-2 rounded-xl">
    ...
    <div className="mb-6 flex items-center justify-between">
      ...
      <button onClick={toggleModal} className="hidden lg:flex gradient text-white rounded-lg btn-sm font-semibold text-sm">Choose </button>
      <button onClick={toggleModal} className="btn btn-sm gradient lg:hidden px-3 text-sm">Starts at {pizza.priceSm}</button>
    </div>
